...
var _ = Describe("Build SQL from a query", func() {
	...
	DescribeTable("query with tags", func(q string, expectedSQL string) {
		...
		Entry("tag with not matcher", "na[amenity=coffee][name!=Starbucks]", `SELECT * FROM entries e JOIN search s ON s.rowid = e.id WHERE (e.osm_type = 'node' OR e.osm_type = 'area') AND s.tags MATCH '( ("amenity coffee") )' AND s.tags MATCH NOT '( ("name Starbucks") )'`),
		Entry("tag should not exist", "na[amenity=coffee][!name]", `SELECT * FROM entries e JOIN search s ON s.rowid = e.id WHERE (e.osm_type = 'node' OR e.osm_type = 'area') AND s.tags MATCH '( ("amenity coffee") )' AND s.tags MATCH NOT '( "name" )'`),
		Entry("everything", `narw[name][!amenity][name="*King*","*Queen*"]`, `SELECT * FROM entries e JOIN search s ON s.rowid = e.id WHERE (e.osm_type = 'node' OR e.osm_type = 'area' OR e.osm_type = 'way' OR e.osm_type = 'relation') AND s.tags MATCH '( "name" ) AND ( ("name *King*") OR ("name *Queen*") )' AND s.tags MATCH NOT '( "amenity" )'`),
	)
})

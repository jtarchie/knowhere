...
var _ = Describe("Build SQL from a query", func() {
	...
	Describe("ToExactSQL", func() {
		...
		DescribeTable("query with tags", func(q string, expectedSQL string) {
			...
			Entry("all tags", `narw[*="*King*","*Queen*"]`, `SELECT * FROM entries e WHERE ( e.osm_type = 'node' OR e.osm_type = 'area' OR e.osm_type = 'way' OR e.osm_type = 'relation' ) AND ( e.tags GLOB '*King*' OR e.tags GLOB '*Queen*' )`),
			...
			Entry("single tag with multiple values", "na[amenity=restaurant,pub,cafe]", `SELECT * FROM entries e WHERE ( e.osm_type = 'node' OR e.osm_type = 'area' ) AND ( e.tags->>'$.amenity' GLOB 'restaurant' OR e.tags->>'$.amenity' GLOB 'pub' OR e.tags->>'$.amenity' GLOB 'cafe' )`),
			Entry("single tag that exists", "na[amenity]", `SELECT * FROM entries e WHERE ( e.osm_type = 'node' OR e.osm_type = 'area' ) AND ( e.tags->>'$.amenity' IS NOT NULL )`),
			...
			Entry("tag with not matcher", "na[amenity=coffee][name!=Starbucks]", `SELECT * FROM entries e WHERE ( e.osm_type = 'node' OR e.osm_type = 'area' ) AND ( e.tags->>'$.amenity' GLOB 'coffee' ) AND NOT ( ( e.tags->>'$.name' GLOB 'Starbucks' ) )`),
